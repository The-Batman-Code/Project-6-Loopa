...
async def get_index():
    # Read the HTML file
    # with open(
    #     os.path.join("D:\Programming\Buildspace\\frontend", "index.html"), "r"
    # ) as html_file:
...
async def chat_reset(request: Request):
    ...
    # Check if the session ID exists before trying to delete
    ...
    # session_id = str(uuid.uuid4())
    # # response = JSONResponse(content={"message": "Chat session reset successfully"})
    # # response.set_cookie(key="session_id", value=session_id)
    # session_id_timers.update({session_id: time.time()})
...
# python code that says hello world
